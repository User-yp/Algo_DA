...
string DecisionTree::guess(vector<string> row) {
	string label = "";
...
int DecisionTree::dfs(vector<string>& row, int here) {
	if (tree[here].isLeaf) {
		return here;
	}
...
double DecisionTree::getEstimatedError(double f, int N) {
	double z = 0.69;
	if (N == 0) {
...
pair<string, int> DecisionTree::getMajorityLabel(Table table) {
	string majorLabel = "";
	int majorCount = 0;
...
bool DecisionTree::isLeafNode(Table table) {
...
int DecisionTree::getSelectedAttribute(Table table) {
	int maxAttrIndex = -1;
...
double DecisionTree::getGainRatio(Table table, int attrIndex) {
	return getGain(table, attrIndex) / getSplitInfoAttrD(table, attrIndex);
}

double DecisionTree::getInfoD(Table table) {
	double ret = 0.0;
...
double DecisionTree::getInfoAttrD(Table table, int attrIndex) {
	double ret = 0.0;
	int itemCount = (int)table.data.size();
...
double DecisionTree::getGain(Table table, int attrIndex) {
	return getInfoD(table) - getInfoAttrD(table, attrIndex);
}

double DecisionTree::getSplitInfoAttrD(Table table, int attrIndex) {
	double ret = 0.0;
...
void DecisionTree::printTree(int nodeIndex, string branch) {
...
string OutputPrinter::joinByTab(vector<string> row) {
	string ret = "";
...
void OutputPrinter::addLine(string str) {
	fout << str << endl;
}
...
//	const char* resultFile = "C:/Users/25069/source/repos/Algo_DA/source/dt_result";
//
